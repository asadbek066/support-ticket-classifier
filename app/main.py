...
from app.audit import get_low_confidence_tickets, log_classification, read_audit_logs
from app.ollama_client import generate_classification
from app.rules import RulesEngine
from app.schemas import BatchClassificationRequest, BatchClassificationResponse, ClassificationResponse, Ticket
...
def runtime_config(config: Mapping[str, object]) -> tuple[list[str], str, str]:
    ...
    categories = [item for item in raw_categories if isinstance(item, str)] if isinstance(raw_categories, list) else []
...
async def classify(ticket: Ticket):
    try:
        ...
        model_out = await asyncio.to_thread(generate_classification, ticket_data, categories, model, api_url)
...
def reload_config():
    try:
        ...
        raise HTTPException(status_code=500, detail="configuration unavailable") from exc
...
def verify_ollama():
    try:
        ...
        raise HTTPException(status_code=503, detail="configuration unavailable") from exc

    payload = {"model": model, "prompt": "test", "stream": False}
    try:
        r = httpx.post(api_url, json=payload, timeout=10.0)
        if r.status_code == 200:
            return {"connected": True, "model": model, "api_url": safe_api_url(api_url), "status": "OK"}
        return {
            "connected": False,
